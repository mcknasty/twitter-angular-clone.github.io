...
class UserRecord extends Record implements BasicRecordInterface {
  ...
  protected static MemberVariblesNames: Array<string>
    = [ 'name', 'handle', 'userName', 'email', 'plainPass', 'token', 'password', 'lastLogin' ];
  ...
  constructor( data?: Partial<UserSchema> ) {
    ...
    if ( data ) {
      if ( UserRecord.instanceOf(data) ) {
        ...
        throw new Error(`Attempted to initialize a User Record with a malformed object: ${dString}`);
      }
    }
    else {
  ...
  public initEmptyRecord(): any {
    let blank = {};
    for(const e of UserRecord.getKeys())
      blank[e] = null;
